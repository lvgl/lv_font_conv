  ...
  lv_compile() {
    ...
    for (let [ format, codepoints ] of subtables_plan) {
      ...
      if (format === 'format0_tiny') {
        ...
        defs = `
static uint8_t glyph_id_ofs_list_${idx}[] = {
${u.long_dump(d)}
};
`.trim();
        ...
        defs = `
static uint16_t unicode_list_${idx}[] = {
...
static uint16_t glyph_id_ofs_list_${idx}[] = {
${u.long_dump(d.ids)}
};
`.trim();
      }
      ...
      const head = `    {
        ...
        .glyph_id_start = ${start_glyph_id}, .unicode_list = ${u_list}, .glyph_id_ofs_list = ${id_list}, .list_length = ${entries_count}
    }`;
