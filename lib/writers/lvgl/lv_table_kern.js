'use strict';


const _    = require('lodash');
const Kern = require('../../font/table_kern');


class LvKern extends Kern {
  constructor(font) {
    super(font);
  }

  create_lvgl_format0_data() {
    let kern_pairs = this.collect_format0_data();

    let kern_pair_glyph_ids = kern_pairs.map(pair =>
      `    {.pair.left = ${pair[0]}, .pair.right = ${pair[1]}},`
    ).join('\n');

    let kern_pair_values = _.chunk(kern_pairs, 8).map(line =>
      '    ' + line.map(pair => this.font.kernToFP(pair[2])).join(', ') + ',\n'
    ).join('');

    return `
/*-----------------
 *    KERNING
 *----------------*/


/*Pair left and right glyphs for kerning*/
static lv_font_fmt_txt_kern_pair_id_t kern_pair_glyph_ids[] =
{
${kern_pair_glyph_ids}};

/* Kerning between the respective left and right glyphs
 * 4.4 format which needs to scaled with \`kern_scale\`*/
static int8_t kern_pair_values[] =
{
${kern_pair_values}};

/*Collect the kern pair's data in one place*/
static lv_font_fmt_txt_kern_pair_t kern_pairs =
{
    .glyph_ids = kern_pair_glyph_ids,
    .values = kern_pair_values,
    .pair_cnt = ${kern_pairs.length},
};


`.trim();
  }

  create_lvgl_format3_data() {
    let f = this.font;
    const { left_classes, right_classes, left_kernings } = this.collect_format3_data();

    function kern_class_mapping(classes) {
      let arr = Array(f.last_id).fill(0);

      classes.forEach((members, idx) => {
        for (let code of members) {
          arr[f.glyph_id[code]] = idx + 1;
        }
      });

      return _.chunk(arr, 8).map(line =>
        '    ' + line.join(', ') + ',\n'
      ).join('');
    }

    function kern_class_values() {
      const arr = [];

      for (let left_class of left_classes) {
        for (let right_class of right_classes) {
          let code1 = left_class[0];
          let code2 = right_class[0];
          arr.push(f.kernToFP(left_kernings[code1][code2] || 0));
        }
      }

      return _.chunk(arr, 8).map(line =>
        '    ' + line.join(', ') + ',\n'
      ).join('');
    }

    return `
/*-----------------
 *    KERNING
 *----------------*/


/*Map glyph_ids to kern left classes*/
static uint8_t kern_left_class_mapping[] =
{
${kern_class_mapping(left_classes)}};

/*Map glyph_ids to kern right classes*/
static uint8_t kern_right_class_mapping[] =
{
${kern_class_mapping(right_classes)}};

/*Kern values between classes*/
static uint8_t kern_class_values =
{
${kern_class_values()}};


/*Collect the kern class' data in one place*/
static lv_font_kern_classes_fmt_txt_t kern_classes =
{
    .class_pair_values   = kern_class_values,
    .left_class_mapping  = kern_left_class_mapping,
    .right_class_mapping = kern_right_class_mapping,
    .left_class_cnt      = ${left_classes.length},
    .right_class_cnt     = ${right_classes.length},
};


`.trim();
  }

  toLVGL() {
    if (!this.font.hasKerning()) return '';

    const format0_data = this.create_format0_data();
    const format3_data = this.create_format3_data();

    if (!format3_data || format0_data.length <= format3_data.length) {
      return this.create_lvgl_format0_data();
    }

    return this.create_lvgl_format3_data();
  }
}


module.exports = LvKern;
