...
class ActionFontAdd extends argparse.Action {
  call(_parser, namespace, value/*, option_string*/) {
...
function parse_args(argv, debug, fs, path) {
  // Do not fall back to process.argv in browserified builds.
  argv = Array.isArray(argv) ? argv : [];
  const isBrowserBundle =
    typeof globalThis !== 'undefined' && globalThis.__LV_FONT_CONV_BROWSER__ === true;
  const metavarPixels = isBrowserBundle ? undefined : 'PIXELS';
  const metavarPath = isBrowserBundle ? undefined : '<path>';
  ...
  let parser = new argparse.ArgumentParser({
    add_help: !isBrowserBundle,
    formatter_class: RawTextHelpFormatter2
  });

  if (isBrowserBundle) {
    // Avoid argparse formatter metavar checks in browserified builds.
    parser._get_formatter = () => ({ _format_args: () => '' });
