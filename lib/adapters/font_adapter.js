 ...
 * @class FontAdapter
 * @abstract
 * Unified interface for font rendering that abstracts away
 * the differences between FreeType, PCF, and BDF fonts
 */
class FontAdapter {
  constructor(sourcePath, sourceBuffer, args) {
    if (new.target === FontAdapter) {
      throw new Error('Cannot instantiate FontAdapter directly.');
    }
  ...
  /**
   * Clean up any resources used by the font adapter.
   * @abstract
   * @throws {Error} if not implemented by subclasses.
   */
  destroy() {
    throw new Error('Method \'destroy()\' must be implemented by subclasses.');
  }
}
