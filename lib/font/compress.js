...
module.exports = function compress(bitStream, pixels, options) {
  ...
  //debug(`input bits: ${pixels.length * opts.bpp} (${pixels.length} pixels)`);
  //debug(`${pixels}`);
  ...
  while (offset < pixels.length) {
    ...
    // Clamp value because RLE counter dencity is limited
    ...
    // If not wnougth for RLE - write as is.
    ...
    // Not reashed state to use counter => dump bit-extended
