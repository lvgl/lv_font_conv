...
class Font {
  constructor(fontData, options) {
    ...
    // 0 => 1 byte, 1 => 2 bytes
    this.glyphIdFofmat = _.max(_.values(this.glyph_id)) > 255 ? 1 : 0;
    debug(`glyphIdFofmat: ${this.glyphIdFofmat}`);

    let kerningValues = _.flatten(this.src.glyphs.map(g => _.values(g.kerning)));
    let kerningBits = _.max(kerningValues.map(k => u.signed_bits(Math.round(k * 16))));
    // 0 => FP4.4, 1 => FP12.4
    this.kerningFormat = kerningBits > 8 ? 1 : 0;
    debug(`kerningFormat: ${this.kerningFormat}`);
