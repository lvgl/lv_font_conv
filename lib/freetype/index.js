...
module.exports.glyph_render = function (face, code, bpp = 8) {
  let glyph_index = FT_Get_Char_Index(face.ptr, code);

  if (glyph_index === 0) throw new Error(`glyph does not exist for codepoint ${code}`);

  let render_mono = (bpp === 1);
  let load_flags = 0;
  load_flags |= m.FT_LOAD_RENDER;
  load_flags |= m.FT_LOAD_FORCE_AUTOHINT;

  if (render_mono) load_flags |= m.FT_LOAD_TARGET_MONO;
  else load_flags |= m.FT_LOAD_TARGET_LIGHT;
  ...
  let pitch  = Math.abs(m.getValue(glyph + m.OFFSET_GLYPH_BITMAP_PITCH, 'i32'));
