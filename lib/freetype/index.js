...
module.exports.glyph_render = function (face, code) {
  ...
  error = m.ccall('FT_Load_Glyph', 'number',
    [ 'number', 'number', 'number' ],
    [ face.ptr, glyph_index, load_flags ]
  );
  ...
  let advance_x = m.getValue(glyph + m.OFFSET_GLYPH_BITMAP_ADVANCE_X, 'i32') / 64;
  let advance_y = m.getValue(glyph + m.OFFSET_GLYPH_BITMAP_ADVANCE_Y, 'i32') / 64;
