...
module.exports = async function collect_font_data(args, createCanvas) {
  ...
  for (let { source_path, ranges } of args.font) {
    let font = fonts_opentype[source_path];
  ...
  for (let dst_code of all_dst_charcodes) {
    ...
    let pixels = ft_render.glyph_render(fonts_freetype[src_font], src_code);

    let glyph_bbox = {
      x: 0,
      y: 0,
      width: pixels.length ? pixels[0].length : 0,
  ...
  let first_font = fonts_opentype[args.font[0].source_path];
