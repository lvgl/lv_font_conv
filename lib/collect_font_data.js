...
const FreeTypeFontReader = require('./font_readers/freetype_font_reader');
const BDFReader = require('./font_readers/bdf_reader');
const PCFReader = require('./font_readers/pcf_reader');

/**
 * create the appropriate FontReader based on file type
 */
function createFontReader(sourcePath, sourceBuffer, args) {
  if (PCFReader.isPCFFont(sourceBuffer)) {
    return new PCFReader(sourcePath, sourceBuffer, args);
  } else if (BDFReader.isBDFFont(sourceBuffer)) {
    return new BDFReader(sourcePath, sourceBuffer, args);
  }

  try {
    return new FreeTypeFontReader(sourcePath, sourceBuffer, args);
  } catch (err) {
...
module.exports = async function collect_font_data(args) {
  ...
  for (let { source_path, source_bin } of args.font) {
    ...
    if (font_readers[source_path]) continue;

    try {
      font_readers[source_path] = createFontReader(source_path, source_bin, args);
    } catch (err) {
  ...
  for (let { source_path, ranges } of args.font) {
    let fontReader = font_readers[source_path];
  ...
  for (let dst_code of all_dst_charcodes) {
    ...
    let fontReader = font_readers[src_font];
    if (!fontReader.hasGlyph(src_code)) continue;

    let glyph_result = fontReader.renderGlyph(src_code, {
  ...
  if (!args.no_kerning) {
    ...
    for (let { code, kerning } of glyphs) {
      ...
      for (let dst_code2 of existing_dst_charcodes) {
        ...
        let krn_value = fontReader.getKerningValue(src_code, src_code2);
  ...
  let first_font_reader = font_readers[first_font_path];

  // Get font metrics through unified interface
  let fontMetrics = first_font_reader.getFontMetrics();
  let underlineMetrics = first_font_reader.getUnderlineMetrics();

  // Clean up FontReader resources
  for (let reader of Object.values(font_readers)) {
    reader.destroy();
  }
  ft_render.destroy();
