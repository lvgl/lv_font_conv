...
module.exports = function collect_font_data(args) {
  ...
  for (let { source } of args.font) {
    ...
    try {
      ...
      throw new AppError(`Cannot load font "${source}": ${err.message}`);
    }
  }
  ...
  for (let { source, ranges } of args.font) {
    let font = fonts[source];

    for (let item of ranges) {
      /* eslint-disable max-depth */
      if (item.range) {
        let chars = ranger.add_range(source, ...item.range);
        let is_empty = true;
        ...
        if (is_empty) {
          ...
          throw new AppError(`Font "${source}" doesn't have any characters included in range ${a}-${b}`);
        }
      }

      if (item.symbols) {
        let chars = ranger.add_symbols(source, item.symbols);
        let is_empty = true;
        ...
        if (is_empty) {
          throw new AppError(`Font "${source}" doesn't have any characters included in "${item.symbols}"`);
        }
      }
    }
  }
